...
func main() {
	...
	// Manually set the scaling factor of the second plaintext to the prime
	// that will be used by the rescaling
	// i.e. params.Q()[min(ct1.Level(), ct2.Level())]
	...
	// Multiply the two complex value vectors
	want := make([]complex128, Slots)
	for i := 0; i < Slots; i++ {
		want[i] = values1[i] * values2[i]
	}

	// Perform the homomorphic multiplication
	eval := hefloat.NewEvaluator(params, evk)
	ct3, err := eval.MulRelinNew(ct1, ct2)
	if err != nil {
		panic(err)
	}

	// Rescale the result by the same scaling factor of the second plaintext
